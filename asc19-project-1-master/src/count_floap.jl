...
function k3_sor_floap(A, b::AbstractVector, w, maxiter)
    fp_1 = flop_counter(0)
    x = zeros(A.n)
    m_sor = copy(A)
    D = DiagonalIndices(A)
...
function k3_ssor_floap(A, b::AbstractVector, w, maxiter)
    x = zeros(A.n)
    m_sor = copy(A)
    D = DiagonalIndices(A)
...
function k3_CB_ssor_floap(A, b::AbstractVector, w, λ_max, λ_min,  maxiter)
    x = zeros(A.n)
    m_sor = copy(A)
    D = DiagonalIndices(A)
    D_t = DiagonalIndices(sparse(A'))
...
function k3_RCS_floap(A, b::AbstractVector, w, maxiter)
    x = zeros(A.n)
    D = DiagonalIndices(A)
...
function k3_JCB_floap(A, b::AbstractVector, maxiter)
    x = zeros(A.n)
    D = DiagonalIndices(A)
...
function k3_GS_floap(A, b::AbstractVector, maxiter)
    x = zeros(A.n)
    m_GS = copy(A)
    D = DiagonalIndices(A)
    fp_1 = flop_counter(0)
